...
b$replicate=0
b.dt = data.table(b) # Many different ways to do this. Am using data.table package for this task. Can use this approach for bootstrapping later.
setkey(b.dt, "replicate")

b.dt[, `:=`(nm = plogis(predict(glm(idu ~ 1, family="binomial"(link="logit")))),
            dn = plogis(predict(glm(idu ~ white + age + cd4, family="binomial"(link="logit"))))),
     by = replicate][, w1:= ifelse(idu==1, nm/dn, (1-nm)/(1-dn))] # add new columns to the data.table b.dt
...
# 1) Merge quintiles of drop out times by replicate to drop out times of individuals
# ...................................................................................
  
  split.1 = function(i){
    quint = c(0,10,as.numeric(quantile(b.dt[b.dt$replicate==i & b.dt$drop==1,]$t, 
  ...
  list.split = split.1(0) # Replicate 0 is the observed data.

  df.split <- list.split # Combine all vectors into a data.frame
  df.split = df.split[order( df.split$id),] # order so I can check in SAS
  df.split[df.split$replicate==0,][1:10,] # check with results in SAS

  colnames(df.split)[which(colnames(df.split) %in% c("time0","t"))] = c("out.t", "in.t")
  df.split.dt = data.table(df.split)
  setkey(df.split.dt, "replicate", "id", "in.t")
  
#  summary(df.split[df.split$replicate==0 ,]$delta) # check with results in SAS
#  summary(df.split.dt[df.split.dt$replicate==0 ,]$delta) # check with results in SAS
#  head(df.split.dt)
  ...
  df.split.dt[, `:=` (nm2 = ifelse(drop==1,
                                   plogis(predict(glm(drop ~ factor(in.t), family="binomial"))),
                                   1 - plogis(predict(glm(drop ~ factor(in.t), family="binomial")))
                                   ),
                      dn2 = ifelse(drop==1, 
                                   plogis(predict(glm(drop ~ factor(in.t) + idu + white + age + cd4, family="binomial"))),
                                   1 - plogis(predict(glm(drop ~ factor(in.t) + idu + white + age + cd4, family="binomial")))
                                   ),
                      by=replicate)] 
  ...
  df.split.dt[, num.do:= cumprod(nm2), by=list(replicate, id)][, den.do:=cumprod(dn2), by=list(replicate, id)] 
  head(df.split.dt)
...
with(df.split.dt, summary(df.split.dt[replicate==0, c("num.do", "den.do", "w2", "w3"), with=F])) #these also match the SAS output.
