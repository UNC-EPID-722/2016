...
data ns;
set ns;
keep patcode bleed contanticoag tobacco_imp contsteroids contaspirin arthrtis age male raceblkoth newcox2;
run;*Only retaining necessary variables;

proc surveyselect data=ns out=ns_boot
        seed=1234 
        method=urs
        samprate=1
        outhits
...
ods select none;
proc genmod data = ns_boot desc;
	by replicate; 
    model bleed = contanticoag tobacco_imp contsteroids contaspirin arthrtis age male raceblkoth newcox2/link=logit dist=bin;
    ods output parameterestimates=betas3(keep=replicate parameter estimate);
run;
ods select all;

proc transpose data=betas3 out=betas4(drop=beta11) prefix=beta; *this makes the set of gamma horizontal;
by replicate;
run;

data ns_pboot;
merge betas4 ns_boot;
by replicate;
run;
...
proc means data = ns_pboot noprint;
	 by replicate;
     var pbleed_1 pbleed_0;
	 output out=me_mleboot mean=upbleed_1 upbleed_0;
run;
...
proc univariate data=me_mleboot1 noprint;
	var rd;
