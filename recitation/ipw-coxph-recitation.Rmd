...
# see http://stackoverflow.com/questions/24585254/working-with-knitr-using-subdirectories
  ...
  opts_chunk$set(fig.width=12, fig.height=8, fig.align="left", echo=T, warning=FALSE, message=FALSE, comment=NA, results="hide")
```
...
See slide 36 of "L3 Cox.pdf" document for more details. Also, you can look at [an R shiny app](https://vonholleunc.shinyapps.io/survival/) that allows you to modify parameters in a Cox model to see different types of proportional hazards assumptions violations.
...
Second test looks at significance of interaction between time and **idu** effect.
...
## ## Check proportional hazards assumption, R

The following link is to an R shiny app where you can view simulations of time interaction with a covariate in a Cox model.

[https://vonholleunc.shinyapps.io/survival/](https://vonholleunc.shinyapps.io/survival/)


## Nice table of Cox PH model assumptions [@harrell_regression_2015, p502]
...
## Calculate crude Cox model by hand (SAS), 2 {.codefont2}
...
## Calculate crude Cox model by hand (R)  {.codefont} 
...
Output the IP weight, $\displaystyle\frac{Pr(IDU=1)}{Pr(IDU=1 \mid \textbf{C})}$ where $\textbf{C}$ = vector of covariates including **white**, **age**, **cd4** as confounders. The **n** data set contains the numerator of stabilized IP weight and the **d** data set contains the denominator of the stabilized IP weight.
...
Merge the data sets with the numerator part of the censor weight, **dn2**, and the denominator part of the censor weight, **nm2**, by the unique person and time combinations. There are 1164 people and up to five time intervals per person so if there were no drop outs and no censoring then there could be up to `r 1164*5` rows in the data set **f**. 
...
$$
\textrm{ipcw}_{it}^{} = \displaystyle \prod_{q=1}^{t} \displaystyle\frac{ Pr(D_i(q)=0)}{Pr(D_i(q)^{}=0 \mid  D_i(q) \geq q, \textrm{cov}_i^{})}
$$
  ...
  * Notation:
    ...
    * $cov_i$ = covariate values for individual $i$
