...
```{r 4-part5b}
# Unweighted regression, crude
coef(summary(glm(bleed~newcox2, family=gaussian(link="identity"), data=ns)))
coef(summary(glm(bleed~newcox2, family=binomial(link="identity"), data=ns)))
```
...
coef(summary(geeglm(bleed~newcox2, family=gaussian, weight=iptw, id=patcode, data=ns)))
coef(summary(geeglm(bleed~newcox2, family=binomial(link="identity"), weight=iptw, id=patcode, data=ns))) # note that you get same results
```
...
coef(summary(geeglm(bleed~newcox2, family=gaussian, weight=smrw, id=patcode, data=ns)))
coef(summary(geeglm(bleed~newcox2, family=binomial(link = "identity"), weight=smrw, id=patcode, data=ns)))
